...
function UI() {
  ...
  const shopTextStopEventListener = (msg: string, data: any) => {
    ...
    // Warning: !!! uncomment this line when finish debugging ShopUI !!!
    // setShowShop(false);
  };
  ...
  function handleShop(event: MouseEvent<HTMLButtonElement>) {
    setShowShop(true);
  }
  ...
  useEffect(() => {
    ...
    tokenShopTextOn = PubSub.subscribe(
      "player:shopText",
      shopTextEventListener
    );
